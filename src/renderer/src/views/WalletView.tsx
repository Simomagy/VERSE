...
import { Wallet, TrendingUp, TrendingDown, SlidersHorizontal, Trash2, Plus, ArrowLeftRight, Pencil, Check, X as XIcon } from 'lucide-react'
import { Button } from '../components/ui/button'
import { Input } from '../components/ui/input'
import { Skeleton } from '../components/ui/skeleton'
import { ContextMenu } from '../components/ui/context-menu'
import { useConfirmDialog, SelectAllCheckbox } from '../components/ui/confirm-dialog'
import { useWalletEntries, useAddWalletEntry, useUpdateWalletEntry, useRemoveWalletEntry, computeBalance } from '../hooks/useWallet'
...
const TYPE_CONFIG: Record<WalletEntryType, {
  label: string
  ...
  income:     { label: 'INCOME',     icon: TrendingUp,        color: 'text-hud-green',  border: 'border-hud-green/40',  bg: 'bg-hud-green/10',  sign: '+' },
  expense:    { label: 'EXPENSE',    icon: TrendingDown,      color: 'text-hud-red',    border: 'border-hud-red/40',    bg: 'bg-hud-red/10',    sign: '−' },
  adjustment: { label: 'ADJUSTMENT', icon: SlidersHorizontal, color: 'text-hud-amber',  border: 'border-hud-amber/40',  bg: 'bg-hud-amber/10',  sign: '=' },
}
  ...
  return (
    ...
    >
      ...
      <div className="flex gap-2">
        <Button variant="hud-ghost" size="sm" className="flex-1" onClick={() => { setOpen(false); setForm(EMPTY_FORM) }}>
          CANCEL
        </Button>
        <Button
          variant="hud"
...
function EntryRow({
  entry, selected, onSelect, onRemove
}: {
  ...
  const [draft, setDraft]     = useState({ amount: '', description: '', category: '', type: entry.type as WalletEntryType })

  function openEdit() {
    setDraft({ amount: String(entry.amount), description: entry.description, category: entry.category, type: entry.type })
    setEditing(true)
  }
  ...
  async function confirmEdit() {
    ...
    await updateEntry.mutateAsync({ ...entry, type: draft.type, amount, description: draft.description, category: draft.category })
    setEditing(false)
  }

  const menuItems = [
    ...(!isAuto ? [{ label: 'Edit Entry', icon: <Pencil className="h-3 w-3" />, onClick: openEdit }] : []),
    { separator: true as const },
    { label: 'Remove', icon: <Trash2 className="h-3 w-3" />, variant: 'danger' as const, onClick: onRemove, disabled: isAuto }
  ]
    ...
    return (
      <div className="border-b border-hud-border/40 px-4 py-3 space-y-2 bg-hud-panel/60">
        <div className="grid grid-cols-3 gap-1.5">
          {(Object.keys(TYPE_CONFIG) as WalletEntryType[]).map((t) => {
            const c = TYPE_CONFIG[t]; const active = draft.type === t
            return (
              <button key={t} type="button" onClick={() => setDraft((d) => ({ ...d, type: t }))}
                className={`py-1.5 border font-mono text-[10px] tracking-widest uppercase transition-all duration-150 ${
                  active ? `${c.border} ${c.bg} ${c.color}` : 'border-hud-border text-hud-muted hover:border-hud-border-glow hover:text-hud-text'
                }`}
              >{c.label}</button>
            )
          })}
        ...
        <div className="grid grid-cols-2 gap-2">
          <div className="relative">
            <Input type="number" min="0" autoFocus value={draft.amount} className="pr-14"
              onChange={(e) => setDraft((d) => ({ ...d, amount: e.target.value }))} />
            <span className="absolute right-2 top-1/2 -translate-y-1/2 hud-label text-hud-dim pointer-events-none">aUEC</span>
          </div>
          <select className="h-9 border border-hud-border bg-hud-deep px-3 text-sm text-hud-text font-mono
        ...
        {(parseFloat(draft.amount) || 0) > 0 && (
          <p className={`hud-label ${editCfg.color}`}>{editCfg.sign} {formatUEC(parseFloat(draft.amount))} aUEC</p>
        )}
        <div className="flex gap-2 pt-1">
          ...
          <button onClick={confirmEdit} disabled={updateEntry.isPending || (parseFloat(draft.amount) || 0) <= 0}
  ...
  return (
    ...
    <div className={`hud-row group grid grid-cols-[1.25rem_2rem_1fr_auto_6rem_8rem_5rem] gap-3 px-4 py-3 items-center
      ...
      <div className="min-w-0">
        <p className="font-mono text-sm text-hud-text truncate">{entry.description || cfg.label}</p>
        <p className="hud-label text-hud-dim">{entry.category}</p>
      </div>

      {/* Badge TRADE — invisible per allineamento se non auto */}
      <span className={`flex items-center gap-1 px-1.5 py-0.5 border hud-label shrink-0
      ...
      <div className="text-right">
        <span className={`font-mono text-sm font-bold ${cfg.color}`}>
          {cfg.sign} {formatUEC(entry.amount)} <span className="text-[10px] font-normal">aUEC</span>
        </span>
      </div>
      ...
      <div className="flex items-center gap-1 justify-end">
        ...
        {/* Remove */}
        {isAuto ? (
          <span title="Managed automatically by trades"
  ...
  return (
    <div className="grid grid-cols-3 gap-2">
      ...
      <div className="hud-panel p-3">
        ...
        <p className={`font-mono text-lg font-bold mt-1 ${balance >= 0 ? 'text-hud-cyan' : 'text-hud-red'}`}>
          {formatUEC(balance)}
        </p>
        <p className="hud-label text-hud-dim">aUEC</p>
      </div>
...
export function WalletView() {
  ...
  const deletableCount = [...selectedIds].filter((id) => !entries.find((e) => e.id === id)?.sourceId).length
  const anySelected    = selectedIds.size > 0

  function handleBulkDelete() {
    confirm(
      { title: 'Delete Entries', message: `Delete ${deletableCount} wallet entr${deletableCount === 1 ? 'y' : 'ies'}? This cannot be undone.` },
      () => {
        [...selectedIds]
  ...
  function askRemoveEntry(entry: LocalWalletEntry) {
    confirm(
      { title: 'Remove Entry', message: `Remove "${entry.description || entry.type}" (${formatUEC(entry.amount)})?` },
    ...
    <div className="relative flex flex-col h-full w-full p-5 gap-4">
      ...
      <div className="flex items-center justify-between hud-section-wallet px-3 py-3 border border-hud-green/20">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-mono text-sm font-bold tracking-[0.15em] text-hud-green uppercase"
        ...
        {FILTERS.map((f) => (
          <button key={f.id}
            className={`px-3 py-1 hud-label transition-colors duration-100 ${
              filter === f.id ? 'text-hud-green border-b border-hud-green' : 'text-hud-muted hover:text-hud-text'
            }`}
      ...
      <div className="grid grid-cols-[1.25rem_2rem_1fr_auto_6rem_8rem_5rem] gap-3 px-4 pb-1">
        <SelectAllCheckbox
          ...
          selected={[...selectedIds].filter((id) => !entries.find((e) => e.id === id)?.sourceId).length}
      ...
      {anySelected && (
        <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-20">
          <button onClick={handleBulkDelete} disabled={deletableCount === 0}
            ...
            DELETE {deletableCount > 0 ? deletableCount : selectedIds.size} ENTR{deletableCount === 1 ? 'Y' : 'IES'}

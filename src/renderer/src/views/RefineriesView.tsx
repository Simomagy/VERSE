...
import { useLocalRefineryJobs, useAddRefineryJob, useUpdateRefineryJob, useRemoveRefineryJob } from '../hooks/useRefineryJobs'
...
function RefineryTimer({ startTime, durationMinutes }: {
  startTime: number
    ...
    return (
      <div className="flex flex-col items-end gap-0.5">
        <div className="flex items-center gap-1">
          ...
          <p className="font-mono text-xs font-bold text-hud-green
            drop-shadow-[0_0_4px_rgba(0,232,122,0.6)]">DONE</p>
        </div>
  ...
  const color = remainingMin < 10
    ? { text: 'text-hud-red',   bar: 'bg-hud-red',   glow: 'shadow-[0_0_6px_rgba(255,64,64,0.7)]' }
    : remainingMin < 30
    ? { text: 'text-hud-amber', bar: 'bg-hud-amber',  glow: 'shadow-[0_0_6px_rgba(232,160,32,0.7)]' }
...
function emptyRow(): MineralRow {
  return {
    key: crypto.randomUUID(), mineral: '', scuInput: '',
    yieldPercent: '', yieldManual: false, scuOutput: '', pricePerScu: ''
  }
}
...
function computeRow(row: MineralRow, uexYields: { commodity_name: string; terminal_name: string; value: number }[], refinery: string) {
  const uexYield = !row.yieldManual && refinery && row.mineral
    ? (uexYields.find(
        (y) =>
  ...
  const scuOut = (row.yieldManual && row.scuOutput !== '')
  ...
  return (
    <div className="space-y-1">
      ...
      {rows.map((row) => {
        ...
        return (
          <div key={row.key} className="grid grid-cols-[1fr_5rem_6rem_5rem_6rem_2rem] gap-2 items-start">
            {/* Mineral */}
            <ComboInput
              value={row.mineral}
            ...
            <div className="relative">
              <Input
                ...
                value={row.yieldManual ? row.yieldPercent : (uexYield !== null ? String(uexYield) : '')}
                ...
                onChange={(e) => {
                  ...
                  const computed = scuIn > 0
                    ? String(Math.round(scuIn * (parseFloat(yp) || 0) / 100 * CSCU * 100) / 100)
                    : ''
              ...
              <button
                type="button"
                title={row.yieldManual ? 'Manual mode — click to switch back to auto (UEX)' : 'Auto mode (UEX) — click to enter manually'}
                className={[
                ...
                onClick={() => onChange(row.key, { yieldManual: !row.yieldManual, scuOutput: '', yieldPercent: '' })}
              >
                {row.yieldManual ? 'M' : 'A'}
              </button>
            </div>
            ...
            {row.yieldManual ? (
              <Input
                ...
                onChange={(e) => {
                  ...
                  const yp = scuIn > 0
                    ? String(Math.round(soSCU / scuIn * 100 * 100) / 100)
                    : ''
              ...
              <div className="h-9 border border-hud-border bg-hud-deep/60 px-2 flex items-center justify-end">
                <span className={`font-mono text-xs ${scuOut > 0 ? 'text-hud-purple' : 'text-hud-dim'}`}>
...
function jobToForm(job: LocalRefineryJob, methods: RefineryMethod[]): { header: JobFormData; rows: MineralRow[] } {
...
function AddJobModal({ onClose, editJob }: { onClose: () => void; editJob?: LocalRefineryJob }) {
  ...
  const mineralSuggestions = useMemo(
    () => commodities
      .filter((c) => c.is_raw === 1 || c.kind === 'Raw')
      .map((c) => c.name),
    [commodities]
  )
          ...
          {totalEstValue > 0 && (
            <div className="grid grid-cols-3 gap-2 pt-1 border-t border-hud-border">
              ...
              <div className="hud-panel p-2 text-center">
                ...
                <p className={`font-mono text-sm font-bold ${netProfit >= 0 ? 'text-hud-green' : 'text-hud-red'}`}>
                  {netProfit >= 0 ? '+' : ''}{formatUEC(netProfit)}
                </p>
              </div>
            </div>
          )}
            ...
            <Field label="REFINE DURATION">
              <div className="flex items-center gap-2">
                <div className="flex-1 relative">
                  ...
                  />
                  <span className="absolute right-2 top-1/2 -translate-y-1/2 hud-label text-hud-dim pointer-events-none">h</span>
                </div>
                <div className="flex-1 relative">
                  <Input
                  ...
                  />
                  <span className="absolute right-2 top-1/2 -translate-y-1/2 hud-label text-hud-dim pointer-events-none">m</span>
                </div>
              </div>
            </Field>
...
function JobRow({
  job, selected, onSelect, onEdit, onSell, onRemove
}: {
  ...
  const menuItems = [
    { label: 'Edit Job',  icon: <svg className="h-3 w-3" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>, onClick: onEdit },
    ...(hasSellable ? [{ label: 'Create Sell Trade', icon: <ShoppingCart className="h-3 w-3" />, onClick: onSell }] : []),
    ...
    <div className={`hud-row group px-4 py-2.5 border-b border-hud-border/40 last:border-b-0
      ...
      <div className="grid grid-cols-[minmax(0,1fr)_auto] gap-4 items-start">
        ...
        <div className="min-w-0 flex gap-3 items-start">
          ...
          <div className="min-w-0 flex flex-col gap-1.5 flex-1">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              <span className="font-mono text-sm font-semibold text-hud-text" title={job.refinery}>
                {job.refinery}
              </span>
              <span className="hud-label text-hud-dim">·</span>
              <span className="hud-label text-hud-purple uppercase tracking-widest" title={job.method}>
                {job.method}
              </span>
              <span className="hud-label text-hud-dim ml-auto">{formatRelativeTime(job.dateAdded)}</span>
            </div>

            {/* Minerali */}
            <div className="flex flex-wrap gap-1">
              {job.minerals.map((m, i) => (
                <div key={i} className="flex items-center gap-1 bg-hud-purple/10 border border-hud-purple/20 px-2 py-0.5">
                  <span className="hud-label text-hud-text" title={m.mineral}>{abbrev(m.mineral)}</span>
                  <span className="hud-label text-hud-dim">
                    {fScu(m.scuInput)} <span className="text-hud-purple">→</span> {fScu(m.scuOutput)} SCU
                  </span>
                  {m.pricePerScu > 0 && (
        ...
        <div className="flex items-center gap-4 shrink-0">
          ...
          {hasValue && (
            <div className="text-right">
              <p className="font-mono text-xs text-hud-cyan">{formatUEC(job.totalEstimatedValue)}</p>
              <p className="hud-label text-hud-dim">EST. VALUE</p>
            </div>
          )}
          ...
          {(hasValue || job.refineCost > 0) && (
            <div className="text-right">
              <p className={`font-mono text-sm font-bold ${isProfit ? 'text-hud-green' : 'text-hud-red'}`}>
                {isProfit ? '+' : ''}{formatUEC(job.netProfit)}
              </p>
              <p className="hud-label text-hud-dim">PROFIT</p>
            </div>
          )}

          <RefineryTimer startTime={job.dateAdded} durationMinutes={job.refineDurationMinutes ?? 0} />
...
function RefineryStats({ jobs }: { jobs: LocalRefineryJob[] }) {
  ...
  return (
    <div className="grid grid-cols-4 gap-2">
      {[
        { label: 'EST. VALUE',    value: formatUEC(stats.totalValue),  unit: 'aUEC', color: 'text-hud-cyan'  },
        { label: 'REFINE COST',   value: formatUEC(stats.totalCost),   unit: 'aUEC', color: 'text-hud-amber' },
        { label: 'NET PROFIT',    value: formatUEC(stats.totalProfit), unit: 'aUEC', color: stats.totalProfit >= 0 ? 'text-hud-green' : 'text-hud-red' },
        { label: 'SCU PROCESSED', value: stats.totalScuIn.toLocaleString(), unit: 'SCU', color: 'text-hud-text' },
      ].map((s) => (
...
export function RefineriesView() {
  ...
  function askRemoveJob(id: string, refinery: string) {
    confirm(
      { title: 'Remove Job', message: `Remove refinery job at "${refinery}"? This cannot be undone.` },
      () => removeJob.mutate(id)
    )
  }
  ...
  return (
    <div className="relative flex flex-col h-full w-full p-5 gap-4">
      ...
      <div className="flex items-center justify-between hud-section-refinery px-3 py-3 border border-hud-purple/20">
        <div className="flex items-center gap-3">
          <FlaskConical
            className="h-5 w-5 text-hud-purple drop-shadow-[0_0_4px_rgba(160,96,255,0.8)]"
          />
          <div>
            <h1
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center flex-1 gap-3">
          <div className="border border-hud-purple/20 bg-hud-purple/5 p-8">
            <TrendingUp className="h-12 w-12 text-hud-purple/40 mx-auto mb-3
      ...
      <AnimatePresence>
        {editJob && <AddJobModal key="edit-job" editJob={editJob} onClose={() => setEditJob(null)} />}
      </AnimatePresence>
      {dialog}
    </div>
  )
